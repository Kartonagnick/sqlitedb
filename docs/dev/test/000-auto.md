...
## lambda  
  - [ ] Тест [работы с лямбдой][19].  
    - [ ] Количество строк - одна:  
      - [ ] Количество аргументов лямды совпадает с количеством столбцов в ответе.  
      - [ ] Количество аргументов лямды меньше, чем количество столбцов в ответе.  
        - [ ] Лишние столбцы игнорируются.  
      - [ ] Количество аргументов лямды больше, чем количество столбцов в ответе.  
        - [ ] В в релизе -> исключение.  
        - [ ] В дебаге -> assert.  
    - [ ] Количество строк больше одной:  
        - [ ] Лямбда возвращает true, и получает на обработку ещё одну строчку.  
        - [ ] Лямбда возвращает false, и обработка строк сразу же завершается.  
    - [ ] База прислала пустой ответ.  
        - [ ] Лямбда не вызывается.  
    - [ ] Лямбды без аргументом запрещена:  
        - [ ] Ошибка компиляции: `not support: zero params of callback`  
