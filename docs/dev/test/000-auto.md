...
## type_safe
  - [x] Тест записи и чтение данных:  
    - [x] Имя: `request/test-type_safe(000)`  
  ...
  - [x] Тест безопасности типов PRIMARY KEY:  
    - [x] Имя: `request/test-type_safe(001)`  
    - [x] Создаём таблицу  
  ...
  - [x] Тест [отсутствия безопасности типов для всех остальных столбцов][14]:  
    - [x] Имя: `request/test-type_safe(002)`  
  ...
  - [x] Тест [отсутствия безопасности типа получателя данных][15]:  
    - [x] Имя: `request/test-type_safe(003)`  
    - [x] Создаём таблицу  
  ...
  - [x] Тест когда аргументов не хватает.  
    - [x] Имя: `request/test-type_safe(005)`  
    - [x] В дебаге -> assert  
    - [x] В релизе -> exception  

  - [x] Тест когда аргументов слишком много.  
    - [x] Имя: `request/test-type_safe(006)`  
    - [x] В дебаге -> assert  
    - [x] В релизе -> exception  
...
## serialize  
  - [x] Тесты использования пользовательского класса в качестве [источника][20] аргументов:  
    - [x] `db::request&& operator <<(db::request&& out, const source& src)`  
    - [x] Количество полей источника правильное (000).  
    - [x] Количество полей источника больше, чем нужно (001).  
      - [x] Должна сработать assert/exception защита.  
    - [x] Количество полей источника меньше, чем нужно (002).  
      - [x] Должна сработать assert/exception защита.  
  - [x] Тесты использования класса в качестве [получателя][21] данных:  
    - [x] База прислала ответ из одной строки:  
      - [x] `void operator >>(db::request&& out, destination& dst)`  
      - [x] Количество полей получателя правильное (003).  
      - [x] Количество полей получателя больше, чем нужно (004).  
        - [x] Должна сработать assert/exception защита.  
      - [x] Количество полей получателя меньше, чем нужно (005).  
        - [x] Это - нормальная ситуация.  
    - [x] База прислала ответ из нескольких строк (006).  
      - [x] В релизе будет обработана только первая строка.  
      - [x] В дебаге сработает assert.  
    - [x] База прислала пустой ответ (007).  
        - [x] Это - нормальная ситуация.  
<br />
<br />

[20]:  ../arch/004-request.md/#специализация-operatort-arg "специализация шаблона для пользовательских типов"  
[21]:  ../arch/004-request.md/#получателем-данных-может-быть-объект-пользовательского-класса   "специализация шаблона для пользовательских типов"  




## container  
  - [x] Тесты работы с [контейнерами][22].  
    - [x] `std::vector<int>`  
    - [x] `std::vector<users>`  
    - [x] `std::vector<std::tuple<int, int>>`  
    - [x] `std::list<int>`  
    - [x] `std::list<users>`  
    - [x] `std::list<std::tuple<int, int>>`  
  - [x] Номинации:  
    - [x] От базы пришел пустой ответ.  
    - [x] От базы пришло множество строк.  
    - [x] От базы пришла одна строка.  
      - [x] Количество полей приёмника равно количеству столбцов в ответе.  
      - [x] Количество полей приёмника больше, чем столбцов в ответе.  
      - [x] Количество полей приёмника меньше, чем столбцов в ответе.  

<br />
<br />

[22]:  ../arch/004-request.md/#Получателем-данных-может-быть-контейнер   "работа с контейнерами"  
